...
module Nat_int : NAT = struct
  ...
  let ( + ) m n = m + n
  let ( - ) m n = if m < n then 0 else m - n
  let ( * ) m n = m * n
...
module Nat_peano : NAT = struct
  ...
  let rec ( + ) x y =
    match x with
      | Nic -> y
      | Nasl x' -> Nasl (x' + y)    (*lahko bi matchal po vseh primerih*)

  let rec ( * ) x y =
    match x with
      | Nic -> Nic
      | Nasl x' -> (x' * y) + y   (*   x*y ---> ((x' +1) * y  *)

  let rec ( - ) x y = 
    match (x, y) with
    | Nic, _ -> Nic    (* zbirsemo y ker ga nismo uporabili*)
    | x, Nic -> x
    | Nasl x', Nasl y' -> x' - y'
  ...
  let rec to_int x =
    match x with
    | Nic -> 0
    | Nasl x' -> Int.add 1 (to_int x') (*to tako naredimo ker je ta nas plus definiran pri peanu*)
...
let fact_5_int = Nat_peano.to_int (
